...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
    ...
    React.useEffect(() => {
        ...
        if (hash && hash.includes('type=recovery')) {
            ...
            setSuccessMsg('Por favor, ingrese su nueva contraseña.');
        }
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isUpdatingPassword) {
                ...
                setSuccessMsg('Contraseña actualizada con éxito. Ya puede iniciar sesión.');
                setIsUpdatingPassword(false);
                setPassword('');
            } else if (isResettingPassword) {
                await authService.resetPassword(email);
                setSuccessMsg('Se ha enviado un correo para restablecer su contraseña.');
    ...
    const getTitle = () => {
        if (isUpdatingPassword) return 'Nueva Contraseña';
    ...
    const getButtonText = () => {
        if (isUpdatingPassword) return 'Actualizar Contraseña';
        if (isResettingPassword) return 'Enviar Correo';
        if (isRegistering) return 'Guardar Registro';
